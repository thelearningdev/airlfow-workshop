...
**What you will learn:** DAG anatomy, `@task` decorator, task dependencies, Variables, branching in the UI.
...
4. On the right, enable the DAG, so that we can run it
5. On the right top, click trigger. 
6. Something fails? 
    - `show_env` to be specific? Why?
    - `check_db` Why?
7. Click on the task, then logs, to see what's going wrong
8. Go to **Admin > Variables** and create a new variable:
   - Key: `bookshop_env`
   - Value: `prod`
9. Set up the `bookshop_postgres` connection in the Airflow UI: Go to **Admin > Connections > + Add**
10. Fill in:
...
12. Trigger the DAG again 
    ...
    - All the `check_db` errors would've vanished
...
| Providers | The imports at the top |
...
1. Open `dags/01_ingest_books_starter.py`

2. **TODO 1** — Read `data/books.csv` and build a list of row tuples:
...
3. **TODO 2** — Insert the rows using `hook.insert_rows`:
...
4. Trigger the DAG. Check the `reconcile` task log — it should print the row count.

5. Run the DAG again, what happens?

6. In what case will we inject the same data over again? What should we do then?

7. Update the task with `replace=True`
...
When we rerun the same task again, with the same set of inputs it should produce the same side effect.
...
4. **TODO 3** — Inside `insert_sales`, load the JSON file using the logical date:
...
```sql
SELECT sale_date, COUNT(*) FROM daily_sales GROUP BY sale_date ORDER BY sale_date;
...
4. Enable both DAGs in the UI. Trigger `03a_producer` manually (it has `schedule=None`).

5. Watch `03a_consumer` fire automatically. Click on the `print_event` task log -- you should see the extra dict printed.
...
4. Enable the DAG with `catchup=True`. After 7 runs you will see `approve_or_reject` tasks waiting.

5. **Play the human reviewer role:**
